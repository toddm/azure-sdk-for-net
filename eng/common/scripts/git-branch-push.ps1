...
if ($SkipCommit -and $AmendCommit) {
    Write-Host "git -c user.name=`"azure-sdk`" -c user.email=`"azuresdk@microsoft.com`" commit --amend -m `"$($CommitMsg)`""
    git -c user.name="azure-sdk" -c user.email="azuresdk@microsoft.com" commit --amend -m "$($CommitMsg)"
    if ($LASTEXITCODE -ne 0)
    {
        Write-Error "Unable to ammend commit message LASTEXITCODE=$($LASTEXITCODE), see command output above."
        exit $LASTEXITCODE
    }
}
